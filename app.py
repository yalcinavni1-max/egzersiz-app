...
# ==========================================
# 1. EKRAN: ANTRENMAN KURULUMU
# ==========================================
if st.session_state.view == "setup":
    st.title("🏋️ Antrenman Planlama")
    st.caption("Bölgelerden o günkü hareketlerini seç, set ve dinlenme sürelerini ayarla.")
    ...
    with st.form("workout_setup_form"):
        ...
        st.divider()
        st.markdown("### Set ve Dinlenme Parametreleri")
...
# ==========================================
    ...
    if st.session_state.rest_mode is None:
        ...
        st.write("Setini bitirdiğinde dinlenmeyi başlatmak için aşağıdaki butona bas.")
        
        is_last_set = st.session_state.current_set >= st.session_state.target_sets
        button_label = "🏁 Hareketi Bitir (Sıradaki Egzersize Geçiş Dinlenmesi)" if is_last_set else "✅ Set Bitti (Set Arası Dinlenme)"
        ...
        if not is_exercise_rest:
            ...
            if st.session_state.current_index + 1 < len(queue):
                ...
                st.balloons()
                st.success("🎉 Tebrikler! Bugünkü tüm egzersizleri ve setleri başarıyla bitirdin!")
